...
##Примеры
1) Простейший вывод списка дочерних ресурсов документа с идентификатором 1:
```
[[pdoResources?
	&parents=`1`
	&depth=`0`
...
2) Если используется дополнительное поле image, то вызов изменится следующим образом:
```
[[pdoResources?
	&parents=`1`
	&depth=`0`
