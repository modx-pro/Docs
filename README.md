...
| `pnpm build` | Продакшен-сборка (перед сборкой запускается генерация OG; для Node задан лимит памяти — см. скрипт в `package.json`); после сборки пишет `.vitepress/dist/redirects.nginx.conf` и падает, если в списке редиректов ошибка |
...
| `pnpm check:redirects` | Список редиректов `.vitepress/config/rewrites.json`: цели существуют, нет цепочек и повторов |
...
- ✅ Переименовали, перенесли или удалили страницу — добавьте редирект со старого адреса в **`.vitepress/config/rewrites.json`**, чтобы не ломать внешние ссылки.
