...
Помимо возможностей, которые даёт markdown и vitepress, на нашем проекте предусмотрена и дополнительная разметка страницы с помощью Frontmatter. Для его добавления вам необходимо в начале файла в блоке между `---` тремя символами дефиса написать пары: название свойства и её значение.
