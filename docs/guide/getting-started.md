...
а также установленный на компьютере менеджер пакетов **[pnpm](https://pnpm.io/installation)**: зависимости зафиксированы в `pnpm-lock.yaml`.
...
| `pnpm run check:redirects` | Редиректы со старых адресов: каждая цель — существующая страница, нет цепочек и повторов |
...
Если вы переименовали, перенесли или удалили страницу, добавьте в `.vitepress/config/rewrites.json` редирект со старого адреса: `"/components/старый-адрес": "/components/новый-адрес"` (без `.md`). Так не сломаются ссылки на страницу с других сайтов и из поиска. CI проверяет только сам список: если страница, на которую ведёт редирект, пропала, проверка не пройдёт, а новую запись нужно добавить самим.
