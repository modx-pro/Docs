...
Убедитесь, что установлен **Node.js 22.18+** (в `package.json` указано `"engines": { "node": ">=22.18" }`). Очистка и переустановка зависимостей: удалите `node_modules` и при необходимости lock-файл, затем снова `pnpm install`.
