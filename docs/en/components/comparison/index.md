---
title: Comparison
description: Product comparison component
logo: https://modstore.pro/assets/extras/comparison/logo.jpg
author: ilyautkin
modstore: https://modstore.pro/packages/ecommerce/comparison

items: [
  { text: 'addComparison', link: 'addcomparison' },
  { text: 'CompareList', link: 'comparelist' },
  { text: 'getComparison', link: 'getcomparison' },
]
---
# Comparison

Paid product comparison component.

Developed for [Simple Dream][1] and distributed via [their repository][2].

Demo: [demo.modx.pro][3]; try before buying on [modhost.pro][4].

[1]: http://simpledream.ru
[2]: https://modstore.pro/packages/ecommerce/comparison
[3]: http://demo.modx.pro/comparison/
[4]: https://modhost.pro
