...
Основной блок отзывов на PDP: карточки отзывов, счётчики, кнопки «полезно», правка и удаление своего отзыва.
...
| `showStats` | `1` | Строка статистики в шапке. На PDP с `msRatingSummary` — `0` |
...
При `showHeading=1` (по умолчанию на standalone PDP) у секции списка появляется `id="msreviews-list-section"`. Chip-фильтры [msReviewsFilters](msReviewsFilters) ведут на этот якорь.
...
Сводку `msRatingSummary` ставьте **над** обёрткой. Параметр `pdoPage` у `msReviews` **не передавайте** — сниппет сам распознаёт вызов из `pdoPage`.
