...
Блок «Частые вопросы» на главной, лендинге, блоге. JSON-LD **не** выводится. На карточке товара используйте [msQuestions](msQuestions), не этот сниппет.

С **1.2.1** при `showProduct=1` заголовок и URL берутся из любого опубликованного `site_content`, не только из `msProduct`.
...
Обёртка `pdoPage` с `element=msQuestionsLatest`. См. [Интеграция — пагинация](../integration#пагинация-pdopage), [msQuestions](msQuestions#пагинация-q-a).

## См. также

- [msQuestions](msQuestions) — Q&A на странице товара
