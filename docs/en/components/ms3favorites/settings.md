...
| `ms3favorites.guest_db_enabled` | Save guest list in DB | `true` | Enable for server-side counter and /wishlist/ (usePdoPage) for guests. Identified by session_id. |
...
- **default** — `max_items`, `storage_type` (frontend, output block).
- **sync** — `guest_db_enabled`, `guest_ttl_days` (DB storage, plugins).
- **share** — `share_ttl_days` (public links).
