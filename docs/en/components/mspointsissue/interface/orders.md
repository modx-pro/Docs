...
When creating an order in [miniShop2][01020103], all component information is stored in the `properties` field.
...
{if $properties.mspointsissue}
  ...
  {if $tmp.point}
    Delivery point: {$tmp.point.name}<br>
  {/if}
  {if $tmp.terminal}
    Delivery terminal: {$tmp.terminal.name}<br>
