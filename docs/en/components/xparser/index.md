---
title: xParser
description: HTML content and RSS/XML feed parser
logo: https://modstore.pro/assets/extras/xparser/logo.png
author: gvozdb
modstore: https://modstore.pro/packages/import-and-export/xparser

items: [
  { text: 'RSS feed parser', link: 'parser-rss' },
  { text: 'HTML content parser + combined tasks', link: 'parser-html' },
  { text: 'Working with miniShop2 products', link: 'minishop2' },
  { text: 'Working with galleries', link: 'gallery' },
  { text: 'Pagination', link: 'pagination' },
  { text: 'Regular expressions', link: 'regex' },
  {
    text: 'Plugin events',
    link: 'events/',
    items: [
      { text: 'xParserOnFilterSourceItems', link: 'events/xparseronfiltersourceitems' },
      { text: 'xParserOnBeforeTaskParse', link: 'events/xparseronbeforetaskparse' },
      { text: 'xParserOnTaskItemParse', link: 'events/xparserontaskitemparse' },
      { text: 'xParserOnBeforeTaskActions', link: 'events/xparseronbeforetaskactions' },
      { text: 'xParserOnTaskParseDone', link: 'events/xparserontaskparsedone' },
      { text: 'Examples', link: 'events/examples' },
    ],
  },
  { text: 'Cron task runner script', link: 'cron' },
  { text: 'Node JS daemon', link: 'nodejs-demon' },
]
---
# xParser
