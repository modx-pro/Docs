...
:::danger
НЕ ВНОСИТЕ ПРАВКИ В ФАЙЛЫ КОМПОНЕНТА. ДЕЛАЙТЕ КОПИИ И ПРАВЬТЕ ИХ. КОПИИ НЕ РАЗМЕЩАЙТЕ В ПАПКАХ КОМПОНЕНТА.
:::
...
В сборке для MODX 3 классы обработчиков используют пространства имён и автозагрузку PSR-4, поэтому примеры ниже отличаются в написании:

* подключать классы через `require_once` не нужно — достаточно `use` нужного базового класса;
* базовые классы называются `FlatFilters\Handlers\Filtering\Resources` (а также `Products`, `Customers`) и `FlatFilters\Handlers\Indexing\Resources`; интерфейсы — `FlatFilters\Handlers\Filtering\FilteringInterface` и `FlatFilters\Handlers\Indexing\IndexingInterface`;
* в файле типов (`types.inc.php`) в поле `className` указывается полное имя класса с пространством имён.

Сам подход (какие методы переопределять, как зарегистрировать тип) остаётся прежним.
:::
