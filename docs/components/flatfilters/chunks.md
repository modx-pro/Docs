...
```
:::tip
Значение опции сортировки задаётся строкой `ключ|направление`. Ключ должен входить в список допустимых (колонки индекса и поля основной таблицы), иначе он будет отброшен. Префикс `Data.` в примере работает только для конфигураций типа «товары»; для ресурсов используйте имя фильтра или `Resource.поле`. Подробнее — в разделе [Сортировка](https://docs.modx.pro/components/flatfilters/#сортировка).
:::
...
```
:::tip
Если из диапазона дат передана только одна граница, фильтр берёт весь день: начало — `00:00:00`, конец — `23:59:59`. Так выбор одной даты возвращает записи за все сутки, а не только за нулевую секунду.
:::
