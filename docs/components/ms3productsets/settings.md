...
| `ms3productsets.cache_lifetime` | TTL кеша подборок (секунды). **`0`** — кеш отключён. Ключ кеша: **`type`**, **`resource_id`**, **`category_id`**, **`set_id`**, **`limit`**, **`exclude_ids`** | `3600` | Продакшен: **> 0** (часто 3600). Для отладки или частых правок — **`0`**. Кешируется результат **`msps_get_products_by_type`** через `cacheManager`. |
