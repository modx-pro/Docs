...
Краткий справочник действий в ms3ProductSets.
...
1. Откройте **Компоненты → Подборки товаров** (вкладка **Список подборок**).
...
1. Перейдите на вкладку **Применение**.
2. Выберите шаблон.
3. Укажите **ID категории (родителя)** в дереве.
4. При необходимости включите **Заменить существующие подборки этого типа**.
5. Нажмите **Применить**.
...
Запись шаблона исчезнет из списка. Связи с тем же `template_name` и `type` удаляются из `ms3_product_sets`.
