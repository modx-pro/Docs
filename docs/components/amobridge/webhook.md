...
```
https://site.ru/assets/components/amobridge/webhook.php?secret=ВАШ_СЕКРЕТ
```

Секретный ключ генерируется автоматически при установке и хранится в настройке **amobridge.webhook_secret**.
...
2. Находится связанный заказ ms3 по таблице `amo_order_link`
3. Ищется обратный маппинг стадии amoCRM → статус ms3
4. Обновляется статус заказа

Для работы обратной синхронизации необходимо:

- Настройка **amobridge.sync_statuses_from_amo** включена
