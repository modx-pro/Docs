...
Перед загрузкой SendIt нормализует имя файла на сервере: приводит его к нижнему
регистру и kebab-case, транслитерирует кириллицу, ограничивает длину и обезвреживает
исполняемые расширения. Например, `Фото договора.PNG` станет
`foto-dogovora.png`, а `shell.php` — `shell-php`. Правило можно переопределить
событием [**siOnSanitizeFileName**](./events#sionsanitizefilename), но результат
плагина всё равно проходит обязательную проверку безопасности.
...
Каждый чанк принимается только для файла, который прошёл валидацию в той же сессии,
с теми же размером, расширением и пресетом. Пути загрузки, удаления и прикрепления
к письму ограничены каталогом текущей сессии.
...
::: info
Пока файл не удалён пользователь имеет возможность добавить этот же файл в очередь загрузки и, если файл не был загружен полностью, загрузка будет продолжена с того места, где была прервана.
:::
