...
| Атрибут | Описание |
|---------|----------|
| `data-fu-wrap` | Блок-обёртка |
| `data-fu-progress` | Блок-обёртка прогресс-бара |
| `data-fu-dropzone` | Блок-обёртка зоны для добавления файлов перетаскиванием (drag'n'drop) |
| `data-fu-list` | Скрытое поле для записи списка загруженных файлов |
| `data-fu-field` | Поле загрузки |
| `data-fu-tpl` | Шаблон превью |
| `data-fu-path` | Указание пути к конкретному файлу |
| `data-fu-hide` | Элемент, который необходимо скрыть после добавления хотя бы одного файла |
...
| `allowFiles` | Содержит имя поля со списком загруженных файлов (в примере `filelist`); именно из этого поля вы сможете получить список файлов для дальнейшей обработки |
...
|-------------|----------|
| `si_msg_loading` | Прогресс загрузки |
| `si_msg_loading_bytes` | Прогресс загрузки не в процентах |
| `si_msg_loaded` | Уведомление об успешном окончании загрузки |
| `si_msg_file_remove_session_err` | Ошибка: ID текущей сессии не содержится в переданном пути |
| `si_msg_files_count_err` | Ошибка: пользователь пытается дополнительно загрузить больше файлов, чем осталось до максимума |
| `si_msg_files_maxcount_err` | Ошибка: пользователь пытается загрузить больше файлов, чем разрешено |
| `si_msg_files_loaded_err` | Ошибка: пользователь пытается загрузить файлы после достижения максимума |
| `si_msg_file_size_err` | Ошибка: превышен максимальный размер файла |
| `si_msg_file_extention_err` | Ошибка: недопустимое расширение файла |
...
|     `actionUrl`     |        **/assets&nbsp;/components&nbsp;/sendit&nbsp;/action.php**        |                путь к файлу-приемнику запроса                |
...
|     `showTime`      |                              **true**                               | позволяет отобразить в консоли браузера время загрузки файла |
