...
'register' => [
  ...
  // Антиспам-защита (с версии 3.1.0)
  'usePoW' => 1, // [!code ++]
...
* `usePoW`: включить Proof of Work (подробнее в разделе [Защита от ботов](https://docs.modx.pro/components/sendit/sending#zashchita-ot-botov-i-vneshnego-dostupa)).
* `useBehaviorSign`: включить поведенческую подпись.
* `minFillTime`: минимальное время заполнения формы в секундах.
