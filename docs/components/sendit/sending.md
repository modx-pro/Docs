...
2. Получив код, пользователь вводит его в соответствующее поле, жмёт кнопку *Подтвердить* и данные снова улетают на сервер, но уже в обработчик пресета *check_code*.
...
::: info
Из коробки компонент не поддерживает капчу, но вы можете добавить её проверку самостоятельно, используя [События](https://docs.modx.pro/components/sendit/events)
:::
