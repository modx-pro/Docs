...
#### siOnSanitizeFileName - генерируется после штатной нормализации имени файла {#sionsanitizefilename}

Событие позволяет проекту изменить итоговое имя до начала загрузки чанков. Доступные
параметры:

* **$name** - исходное имя из браузера.
* **$sanitized** - имя после штатной нормализации SendIt.
* **$extension** - нормализованное расширение.
* **$directory** - каталог загрузки текущей сессии.
* **$context** - контекст операции, для загрузки имеет значение `upload`.
* **$formName** - имя формы.
* **$presetName** - имя пресета.
* **$SendIt** - экземпляр класса-обработчика.

Новое имя возвращается через `$modx->event->returnedValues['name']`. Можно вернуть
относительный подкаталог, но нельзя выйти за каталог сессии. Результат плагина всегда
повторно фильтруется: управляющие символы и traversal удаляются, исполняемые расширения
обезвреживаются, длина ограничивается.

::: details Пример плагина

```php:line-numbers
switch ($modx->event->name) {
  case 'siOnSanitizeFileName':
    $modx->event->returnedValues['name'] = 'contracts/' . $sanitized;
    break;
}
```
...
* **$webConfig** - массив параметров конфигурации, включая `protectionMap` — карту антиспам-защиты по пресетам (с версии 3.1.0).
...
::: tip
С версии 3.1.0 `webConfig` содержит ключ `protectionMap` — массив с информацией о включённых механизмах защиты для каждого пресета.
Вы можете изменить карту защиты в этом событии, например отключить PoW для определённых пресетов:

```php:line-numbers
switch ($modx->event->name){
  case 'senditOnGetWebConfig':
    // Отключить PoW для пресета 'feedback'
...
#### si:send:reset:after - после сброса формы

Событие возникает ПОСЛЕ сброса формы (вызова `target.reset()` или очистки значения поля). Может быть отменено.

::: details Передаваемые параметры

* **target** - может быть формой или полем.
* **Sending** - объект класса *Sending* для быстрого доступа к методам и свойства этого класса.
  :::
...
document.addEventListener('si:send:reset:after', (e) => {
    ...
    // выполним дополнительные действия после сброса
...
#### si:quiz:init - инициализация опросника

Событие возникает при инициализации каждой формы опросника, ДО сброса формы и настройки прогресс-бара. Не может быть отменено.

::: details Передаваемые параметры

* **items** - массив элементов-шагов.
* **root** - элемент формы.
* **currentIndex** - индекс текущего шага (из cookie или 1 по умолчанию).
...
document.addEventListener('si:quiz:init', (e) => {
    ...
    // добавим кастомную нумерацию шагов
...
Событие возникает при сбросе опросника. Событие не может прервать сброс.
...
* **currentQuestion** - элемент текущего вопроса.
...
### События Уведомлений (Notify)

---

#### si:notify:before - перед показом уведомления

Событие возникает ПОСЛЕ формирования параметров уведомления, но ДО его отображения. Не может быть отменено.

::: details Передаваемые параметры

* **options** - объект параметров уведомления (`type`, `title`, `upd` и настройки из `handlerOptions`).
* **Notify** - объект класса *Notify* для быстрого доступа к методам и свойства этого класса.
:::
