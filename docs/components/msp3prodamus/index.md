...
- **URL страницы.** Адрес платёжной страницы из адресной строки, обычно `https://имя.payform.ru/`. Это `msp3prodamus_payform_url`.
- **Секрет.** Войдите на платёжную страницу, нижнее меню **Настройки**, скопируйте секретный ключ. Новый ключ в кабинете не выпускается, его выдаёт поддержка Prodamus. Инструкция: [где найти секрет и URL уведомлений](https://help.prodamus.ru/payform/integracii/rest-api/url-dlya-uvedomlenii-i-sekretnyi-klyuch).
- **sys.** Код интеграции. Его согласуют с поддержкой Prodamus. Один код на всю интеграцию. Без него Prodamus не примет `urlNotification` из запроса ссылки.
- **URL уведомлений.** На той же странице настроек поле **Настройка уведомлений**. После ввода нажмите **сохранить**.
