...
| **ignoreGroups** | | Группы, опции которых не нужно выводить в списке, через запятую. |
| **groups** | | Выводить опции только указанных групп (название или идентификатор категории через запятую) |
| **sortGroups** | | Порядок сортировки групп опций. Принимает как ID, так и названия. Например: `22,23,24` или `Размеры,Электроника,Разное`. |
| **sortOptions** | | Порядок сортировки опций. Передаются строкой, например: `size,color`. |
