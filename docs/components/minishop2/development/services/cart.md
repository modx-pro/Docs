...
- **add($id, $count = 1, $options = [])** — Добавляет товар в корзину. Обязателен параметр `id`, опционально `count` и `options`.
...
- **status** — Возвращает статус корзины: `total_count`, `total_cost`, `total_weight`, `total_discount`, `total_positions`
...
switch ($modx->event->name) {
  ...
  // Статус корзины
  case 'msOnGetStatusCart': break; // получает $status и $cart
}
```

Во всех методах присутствует объект **$cart** - это сам класс msCartHandler, со всеми методами.

Переменная `$key` это ключ элемента корзины. Формула зависит от системной настройки `ms2_cart_product_key_fields` (по умолчанию `id,options`):

```php
'ms' . md5($id . json_encode($options));
// где $id - это идентификатор товара, а $options - массив опций, присланных при добавлении
```

Ключ всегда имеет префикс `ms`. Поля для генерации ключа можно изменить через настройку `ms2_cart_product_key_fields`.
...
Еще в этом событии есть массив `$options` — дополнительные параметры (опции), присланные юзером при добавлении товара в корзину.
