...
<!-- ![Утилиты → Импорт](/components/minishop3/screenshots/mgr-import.png) -->

Пошаговый мастер импорта товаров из CSV.

## Назначение

Массово создаёте и обновляете товары из CSV:

- Автоопределение кодировки (UTF-8, Windows-1251, KOI8-R)
- Сопоставление колонок с полями товара
- Обновление по ключевому полю
- TV, опции (`option_*`), изображения в галерею
- Активные **дополнительные поля** (`msExtraField`) для `msProduct` / `msProductData`
...
**Разделители:** `;` (точка с запятой, рекомендуем), `,` (запятая), `Tab`.

После загрузки увидите число строк, кодировку и предупреждение для большого файла.
...
**Обязательные поля:** `pagetitle` (название), `parent` (ID категории).
...
Ключи поиска: `article` (лучше), `pagetitle`, `id`.

### Дополнительные поля и Object Extension

Список колонок для маппинга собирает `GET /api/mgr/import/fields`:

1. Статический каталог `config/import-fields.php` (ресурс, `msProductData`, опции).
2. Активные записи `msExtraField` для моделей `msProduct` и `msProductData`, у которых колонка уже есть в БД (`column_exists`).

Тип виджета `ms3-repeater` в импорт **не попадает**: JSON-строки репитера через CSV не маппятся. Простые extra fields (`varchar`, `int`, `decimal`, `text`, …) и key-value (если колонка есть) выбираются в UI как обычные поля товара.

Зарезервированные имена из статического каталога нельзя перекрыть ключом extra field.

Опции по-прежнему задают префиксом `option_` в CSV или явным маппингом на ключ опции.
...
| Режим | Описание |
| --- | --- |
| Синхронный | Импорт идёт сразу, вы ждёте ответ |
| Асинхронный | Задача уходит в [Scheduler](/components/scheduler/) (большие файлы) |
...
| `color` | Цвет |
| `size` | Размер |
| `stock` | Остаток (`remains` — alias в CSV) |
...
- `msOnBeforeImport`: перед стартом
- `msOnImportRow`: на каждой строке
- `msOnAfterImport`: после завершения
