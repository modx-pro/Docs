...
::: warning Расхождение версии и файлов
...
::: warning Неуспешная оплата и складской резерв
...
::: warning Ошибка в формате запрещает все переходы
...
::: danger Перед включением заполните остатки
...
::: tip Почему «доставлено» по умолчанию ничего не меняет
...
Настройки личного кабинета. Как это выглядит на фронтенде — [Вход и регистрация](/components/minishop3/frontend/customer-auth).
...
::: warning Пустое значение отмену не запрещает
...
Настройки Web API (`api.php`). Список методов — [REST API](/components/minishop3/development/api).
...
::: warning Неверный JSON игнорируется молча
...
Параметры событий и порядок их вызова — в разделе [События товара](/components/minishop3/development/events/product).
