...
Страницы ЛК и поведение регистрации. Витрина: [Вход и регистрация](/components/minishop3/frontend/customer-auth).

`login_page_id` / `register_page_id` задают URL в ссылках. Формы по умолчанию рендерит `msCustomer` через `unauthorizedTpl`, отдельные чанки «только login» в пакете нет.
...
`auto_register_on_order` / `auto_login_on_order`: гость при submit может стать `msCustomer` без формы ЛК. Выключите, если аккаунт создаёте только вручную.

`require_email_verification`: письмо со ссылкой на `GET /api/v1/customer/email/verify`. Пока email не подтверждён, часть сценариев ЛК может требовать повторную отправку.
...
Чтобы запретить отмену заказов покупателями, оставьте настройку пустой.
:::

### Синхронизация с modUser

По умолчанию выключено. ЛК работает на `msCustomer` и токене MS3. Включайте sync, если нужны группы MODX, ACL или общие сессии с другими компонентами.
