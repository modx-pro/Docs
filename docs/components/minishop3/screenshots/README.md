...
Положите PNG в эту папку. В markdown уже стоят пути `/components/minishop3/screenshots/имя-файла.png` (заглушки без файлов). После добавления PNG правки текста не нужны.

| Файл | Что снять | Где используется |
| --- | --- | --- |
| `mgr-orders.png` | **Extras → MiniShop3 → Заказы**: список, переключатель черновиков | [interface/index.md](../interface/index.md) |
| `mgr-order.png` | Карточка заказа (вкладки, пересчёт стоимости) | [interface/index.md](../interface/index.md) |
| `mgr-customers.png` | **Extras → MiniShop3 → Клиенты** | [interface/index.md](../interface/index.md) |
...
| `mgr-category-products.png` | Таблица товаров на ресурсе категории (колонки-опции) | [interface/category.md](../interface/category.md) |
