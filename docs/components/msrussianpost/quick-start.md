...
2. Найдите пакет **msRussianPost** в каталоге [ModStore](https://modstore.pro/) (**Download Extras**) или загрузите архив транспортного пакета вручную, если пакет собран локально
...
- **25** системных настроек (фильтр в **Настройки → Системные настройки**: `msrussianpost`)
- пункт меню **Extras → Почта России**
- сниппеты `msRussianPost` и `msrpLexiconScript`
- чанки `tplRussianPostStatus` и `tplRussianPostMethods`
- плагин **msRussianPost Autoload** на событие **`OnMODXInit`** (ранняя загрузка класса доставки для MiniShop3)
- плагин **msRussianPost Delivery** на событие **`msOnGetDeliveryCost`**
- плагин **msRussianPost Order tariff** на события **`msOnSubmitOrder`**, **`msOnBeforeCreateOrder`**, **`msOnCreateOrder`** — переносит выбранный код тарифа в свойства заказа для отображения в карточке заказа в менеджере MiniShop3
...
2. Поля:
   ...
   - **class**: `msrussianpost\Delivery\RussianPostDelivery`
...
![MiniShop3 — настройка способа доставки Почта России](/components/msrussianpost/screenshots/minishop3-delivery-russianpost.png)

![MiniShop3 — настройка способа доставки Почта России](/components/msrussianpost/screenshots/minishop3-delivery-russianpost-2.png)

## Шаг 2: Системные настройки

**Настройки → Системные настройки**, фильтр: `msrussianpost`.

Минимум:

| Ключ | Действие |
|------|----------|
| `msrussianpost_delivery_id` | Укажите ID из шага 1 (несколько ID — через запятую). Пусто: в JS подставляются все доставки с классом `msrussianpost\Delivery\RussianPostDelivery` (**auto**). Если таких нет — виджет может показываться при любой доставке (**any**). Для рабочего сайта задайте ID явно |
| `msrussianpost_sender_index` | Индекс отправителя, 6 цифр (например индекс вашего склада) |
...
**Порядок:** сначала лексикон для JS, затем сниппет (он подключает CSS и `russianpost.js` и выставляет `window.msRussianPostConfig`), затем чанки статуса и методов.

::: tip Подсказки адреса и индекс (mxDadata)
Чтобы покупатель выбирал адрес из подсказок DaData и в заказ попадал индекс, установите **mxDadata** и выведите **`[[!mxDadataAddressSuggest]]`** (или Fenom-эквивалент) **выше** блока с `msrpLexiconScript` / `msRussianPost`. Порядок вывода и событие **`mxdadata:order-address-updated`** — в разделе [Подключение на сайте → mxDadata](frontend#mxdadata).
:::

![Блок виджета Почты России](/components/msrussianpost/screenshots/msrp-block.png)
...
Параметры сниппета `msRussianPost`: `connectorUrl`, `indexSelector`, `debug` — см. [msRussianPost](snippets/msRussianPost).
...
- [Сниппеты](snippets/) — `msRussianPost`, `msrpLexiconScript`
- [Подключение на сайте](frontend) — коннектор, чанки, стили, [mxDadata](frontend#mxdadata)
