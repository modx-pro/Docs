...
JWT и `customerCode` положите в `msPayment.properties` (`jwt_token`, `token`, `secret` или `secret_key`). Если properties пусты, пакет читает системные настройки. Пустой JWT пакет берёт из устаревшего `msptochka_jwt_token`.
...
3. **Создать JWT-ключ**. Задайте название и срок. После срока ключ перевыпускают вручную. Отдельного refresh-токена нет.
4. **Продолжить**. Отметьте разрешения. Банк для эквайринга обычно требует `MakeAcquiringOperation` и `ReadAcquiringData`. Пакет права токена не проверяет. Несколько компаний: разрешения у каждой. `ManageWebhookData` нужен только при регистрации webhook через API Точки. Из админки MODX пакет webhook не регистрирует.
5. **Создать ключ** и код из СМС.
6. Скопируйте JWT в **`msp3tochka_jwt_token`**. Тот же токен лежит в блоке **JWT-ключи** того же сервиса.

На том же экране банк показывает `client_id`. В настройках msp3Tochka поля для него нет. Нужен при регистрации webhook через API Точки: один URL на один `client_id`.

При перевыпуске банк удаляет старый JWT и старый `client_id`. В MODX вставьте новый токен. Webhook со старым `client_id` зарегистрируйте заново.

В запрос пакет сам подставляет заголовок `Authorization: Bearer`.

## customerCode

Это идентификатор компании в API. Банк обычно даёт строку до 9 символов. Пакет длину не проверяет. На экране создания JWT его нет.
...
Если точек несколько, банк требует `merchantId`. По документации банка это 15 цифр из [Get Retailers](https://developers.tochka.com/docs/tochka-api/api/get-retailers-acquiring-v-1-0-retailers-get). Пакет формат не проверяет. Идентификатор точки СБП другой: по правилам банка он начинается с `MA` или `MB`. Его в эту настройку не кладите.
...
2. Либо откройте способ оплаты и заполните properties: `jwt_token` (также `token`, `secret`, `secret_key`) и `customer_code`. Непустые properties перекрывают системные настройки.
...
1. **`msp3tochka_test_mode`** = Нет. Хост в боевом режиме: `https://enter.tochka.com/uapi`, если не задали `msp3tochka_api_base_url`.
