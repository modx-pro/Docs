...
| `mxapi.cursor_secret` | создаётся при установке | Ключ подписи курсоров постраничного обхода. Значение на каждом сайте своё; курсор без подписи был бы произвольным вводом в параметрах выборки. Пустое значение **выключает** курсорную пагинацию (`internal_error`), а не разрешает неподписанные курсоры |
