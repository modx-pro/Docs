...
::: tip Если эндпоинт предназначен для регулярных выгрузок
Двум вещам стоит найти место сразу, пока контракт не разошёлся по интеграторам: `'cache' => EndpointMetadata::CACHE_ETAG` — и клиент узнаёт «ничего не изменилось» ответом `304` без тела; параметр `cursor` с `readCursor()` / `nextCursor()` — и большая выборка проходится страницами без задвоений при вставках. В курсорном режиме `meta.total` не отдаётся намеренно: подсчёт всей выборки на каждой странице стоит дороже самой выгрузки, вместо него клиент получает `meta.has_more`. Оба механизма — [Инкрементальные выгрузки](endpoint-reference#etag).
:::
...
### Свой промежуточный обработчик

Если пакету нужно вмешаться не в один эндпоинт, а в каждый запрос — сверить подпись вебхука, добавить заголовок, отмерить время, — это промежуточный обработчик (`MxApi\Core\Middleware\MiddlewareInterface`). Регистрируется он так же, как провайдер, только событием `mxApiOnRegisterMiddleware`:
...
Обработчики сайта по-прежнему объявляются ключом `middleware` в `core/config/mxapi.php`. Порядок цепочки фиксирован: встроенные обработчики пакета → обработчики пакетов → обработчики сайта → эндпоинт. Подробнее — [Промежуточные обработчики](endpoint-reference#middleware).
...
- [Контракт и совместимость](contract) — на что из ядра можно опираться и что будет с версиями.
