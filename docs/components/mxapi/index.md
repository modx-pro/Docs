---
title: mxApi
description: Единая точка входа публичного API для MODX Revolution 2 — маршруты под своим префиксом, bearer-токены, scope поверх прав MODX, каталог эндпоинтов и OpenAPI из живого реестра.
outline: [2, 3]
lastUpdated: true
author: ShevArtV
items: [
...
**Единая точка входа публичного API для MODX Revolution 2.** Пакет даёт транспорт, аутентификацию по bearer-токенам, реестр эндпоинтов, каталог и выгрузку OpenAPI. Сами эндпоинты он не поставляет — их приносят **провайдеры**: другие пакеты или код конкретного сайта.
...
Для MODX 3 предназначена отдельная линейка пакета: ядро у них общее, различаются только платформенные адаптеры.
