...
Блок «Вопросы и ответы» на PDP. Поддерживает pdoPage и фильтр `faqOnly` (только вопросы с ответом).
...
Параметр `pdoPage` у `msQuestions` **не передавайте**. Отдельный `pageVarKey`, не совпадающий с отзывами:
