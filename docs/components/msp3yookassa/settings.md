...
| Ключ | Тип | Описание |
|------|-----|----------|
| `msp3yookassa_debug` | да/нет | Включить служебные сообщения компонента в лог MODX (`LOG_LEVEL_DEBUG`): запрос `createPayment`, webhook, чеки, неудачный capture. **По умолчанию выкл.** — в штатном режиме лишнего шума в логе нет. Сообщения о критических ошибках (неверные ключи, пустой `confirmation_url`, ошибка сохранения заказа после создания платежа) по-прежнему возвращаются вызывающему коду (JSON API заказа), без обязательной записи в лог. |
