...
2. Выбор ПВЗ из списка или на карте. Поддерживаются два провайдера карт: **OpenStreetMap + Leaflet** (бесплатно, без ключа, по умолчанию начиная с 2.9.0) и **Яндекс.Карты** (требует [ключ для JavaScript API v3](https://developer.tech.yandex.ru/)).
3. Подсказки при вводе адреса от сервиса DaData.
4. Сохранение полного объекта выбранного ПВЗ в заказе и вывод его адреса в карточке заказа miniShop2 (начиная с 2.9.3).
...
Начиная с версии 2.9.3 компонент сохраняет в `msOrderAddress.properties` два значения:

- `point` — выбранное значение поля ПВЗ;
- `pointData` — полный объект ПВЗ СДЭК, включая `code`, `location.address`,
  `location.address_full`, координаты и режим работы.

В карточке заказа miniShop2 выводятся выбранное значение и полный адрес ПВЗ. Карта и
смена ПВЗ из админки не предусмотрены.

### Вывод полного адреса ПВЗ в письме

В Fenom-чанке письма полный адрес выбранного ПВЗ можно получить из свойств адреса заказа:

```fenom
{$address.properties.pointData.location.address_full ?: $address.properties.pointData.location.address ?: $address.properties.point}
```

Сначала используется `address_full`, затем обычный адрес из `location.address`. Последний
запасной вариант — сохранённое значение поля `point`, в том числе для старых заказов без
`pointData`.
...
4. Убедитесь, что в поле ms_cdek2_deliveries установлены правильные ID способов доставки (door — доставка до двери, pvz — доставка в ПВЗ);
   :::details
   Например ID доставки до двери у вас 9, а до ПВЗ 10, тогда значение этой настройки будет выглядеть так `{"door":"9","pvz":"10"}`.
   :::
...
Начиная с версии 2.9.0 поддерживаются два провайдера карт. Выбор задаётся системной настройкой `ms_cdek2_map_provider`:
...
::: info
До версии 2.9.0 ключ Яндекс.Карт хранился в `ms_cdek2_yandex_map_api_key`. При обновлении пакета значение автоматически переносится в `ms_cdek2_map_api_key`, а `ms_cdek2_map_provider` выставляется в `yandex`. Если до этого ключа не было — `ms_cdek2_map_provider` останется `osm`. Старая настройка удаляется.
:::
...
Отдельными системными настройками можно управлять учётом количества (`ms_cdek2_use_product_count`), индивидуальных размеров товаров (`ms_cdek2_use_individual_sizes`) и индивидуального веса товаров (`ms_cdek2_use_individual_weight`) при расчёте количества пакетов.
По умолчанию все застройка имеют значение `false`, т.е. при расчёте количества пакетов не учитывается ни реальное количество, ни индивидуальные размеры, ни вес, а берутся значения по умолчанию (из системных настроек и количество = 1).

::: warning
Для способов расчёта количества пакетов `0,1,2,3` в системной настройке `ms_cdek2_default_size` следует указывать размер упаковки одного товара, а для способов `4,5` - размеры всего пакета.
...
::: warning
miniShop2 не поддерживает работу с полями типа `select`, т.е. если вы хотите сделать выбор страны через такое поле, нужно написать дополнительный код на JavaScript.
