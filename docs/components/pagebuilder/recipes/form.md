...
1. В CMP **Forms** создайте схему с ключом и полями. Типы: `text`, `email`, `tel`, `textarea`, `select`, `radio`, `checkbox`, `date`, плюс `hidden` и `consent`. Для select и radio заполните **Варианты**.
