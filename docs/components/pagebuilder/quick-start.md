...
| VueTools | 1.1.2+ |
| pdoTools | 3.0+ |
| PageBuilder (Free) | 1.0.12-beta |
| PageBuilder Pro | 1.0.12-beta, требует `pagebuilder` ≥ 1.0.12 |
...
3. Сохраните ресурс MODX (**Сохранить** в toolbar ресурса). Редактор проверяет обязательные поля, пишет черновик и публикует секции на сайт.
4. Автосохранение во вкладке пишет только черновик. На витрине его нет, пока не сохраните ресурс.
...
- ресурс не сохранили после правок (autosave не публикует)
- у сниппета нет прав на ресурс
- в шаблоне закэширован вызов без `!`. Используйте `[[!PageBuilder]]`

Секции `quiz` и `contact_form` (Pro) на витрине нуждаются в **FetchIt**. Без него форма покажет сообщение о недоступности.
