...
Свой вывод таблицы в шаблоне, виджете или отдельном chunk. Секция Pro [data_table](../sections/data_table) использует встроенный chunk `pagebuilder_data_table`. Этот сниппет нужен, когда разметку пишете сами.
...
<!-- MEDIA: output | must | пример JSON-массива строк таблицы (поле data у каждой записи) | [[!PageBuilderTableRows? &table_key=`…` &limit=`5`]] на ресурсе с синтетической таблицей на стенде -->
