...
- `<lastmod>` — `editedon` ресурса. Если поле пустое, берётся `publishedon`. Если и он пустой, время запроса. xPDO отдаёт дату и числом, и строкой. Сниппет приводит оба вида к unix-времени и пишет ISO 8601
