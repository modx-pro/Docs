...
Подписи фильтров берутся из `label` колонки схемы таблицы, не из ключа поля. Колонка типа `image` не попадает в фильтр и сортировку. После отправки форма остаётся на `#pb-{id}`. Номер страницы больше последней приводится к последней.
