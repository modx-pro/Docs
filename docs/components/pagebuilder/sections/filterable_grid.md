...
Операторы запроса те же, что у [динамического списка](dynamic_list): `eq`, `contains`, `in`, `gte`, `lte`, `between`, `empty`, `not_empty`. `contains` не различает регистр, включая кириллицу. Ключи `sql`, `php`, `snippet` и `class` отклоняются. Страница не больше 100.

Подписи фильтров берутся из `label` колонки схемы таблицы, не из ключа поля. Колонка типа `image` не попадает в фильтр и сортировку. В форме тип поля задаёт оператор: text → `contains`, number и currency → `gte`, color → `eq`. Номер страницы больше последней приводится к последней.
