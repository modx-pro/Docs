...
Компонент msMCD (mini cart dynamic) для miniShop2 — это рефакторинг старого компонента msMiniCartDynamic с рядом улучшений и предназначен для обновления мини корзины на лету — динамически. Поддержка msMiniCartDynamic прекращена.
...
| **animate**     | `false`                 | Анимированное добавление в корзину. У изображения, в чанке вывода списка товаров необходимо добавить класс `msmcd-img`. Также изображение должно находиться внутри тега `form` |
