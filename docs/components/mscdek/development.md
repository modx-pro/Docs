...
* `mscdek_packages_sizes_ceil` — округление вверх до целого сантиметра после
  множителя; выключено по умолчанию. Пустые размеры товара используют размеры
  упаковки по умолчанию.
* `mscdek_main_api_url` — URL API (по умолчанию `https://api.cdek.ru/v2/`).
  Для тестового контура укажите `https://api.edu.cdek.ru/v2/` и тестовые реквизиты
  в `mscdek_main_login` / `mscdek_main_password`. OAuth-кеш разделён по URL и логину.
* `mscdek_main_log_level` — порог `debug`, `info`, `warning` или `error`
  (по умолчанию `error`). Логирование включается `mscdek_main_debug` и использует
  mxLogger для MODX 3; старые файлы логов больше не пополняются. Без mxLogger
  при попытке записи выводится одно предупреждение за экземпляр логгера.
* События `msCdekOnGetProductWeight` и `msCdekOnGetProductSizes` позволяют брать
  значения из Опций; [параметры и пример плагина](events).
* В составной адрес без `text_address` включаются `building`, `room`, `metro`;
  индекс передаётся отдельно. При пустом городе поиск ПВЗ дополнительно использует
  города СДЭК по индексу и DaData (для последнего нужен настроенный токен).
* HTTP-клиент сохраняет метод DELETE и бинарные PDF; содержимое PDF и OAuth-ответов
  не пишется в отладочный лог. Данные выбранного ПВЗ сохраняются при пересчёте.

Для наследников API-клиента сохранены `getApiUrl()`, `getTokenCacheKey()` и
`isDebugEnabled()`. В боевом режиме наследник с `parent::getTokenCacheKey()`
автоматически использует новый ключ, учитывающий URL и логин.
Совместимые вызовы `new Logging($debug)` и `setPath()` сохранены; `setPath()`
больше не меняет файл. При создании сервиса вне обычного MODX-запроса передавайте
MODX явно: `new Logging($debug, $modx)`.
