...
#### msCdekOnSetTariffCode - генерируется на этапе получения кода тарифа, позволяет его подменить

Доступные параметры:

* **$delivery** - объект класса msDelivery.
* **$object** - экземпляр класса MsCdek\MsCdek.
...
#### msCdekOnSetRequestData - генерируется перед отправкой запросов в API, позволяет дополнить или изменить данные запроса
...
#### msCdekOnGetWebConfig - генерируется при добавлении JavaScript компонента на страницу, позволяет прокинуть любые параметры в JavaScript
...
#### msCdekOnSendRequest - генерируется после отправки запроса в API СДЭК, позволяет обработать или модифицировать ответ

Доступные параметры:

* **$requestData** - массив данных запроса.
* **$method** - имя метода API.
* **$response** - массив ответа от API.
* **$object** - экземпляр класса MsCdek\MsCdek.
...
```php:line-numbers
switch ($modx->event->name){
  case 'msCdekOnSendRequest':
    // логируем все запросы к API СДЭК
    $modx->log(1, 'CDEK API ' . $method . ': ' . json_encode($response));
    break;
}
```
...
#### msCdekOnResetStatus - генерируется после сброса некоторых данных статуса доставки
...
switch ($modx->event->name){
  case 'msCdekOnResetStatus':
       unset($_SESSION['mscdek']['data']['custom_param']);
    break;
}
```
...
подписывайтесь на это событие, так как после его срабатывания объект **mscdek** и его дочерние элементы точно доступны.
...
#### mscdek:point:change - выбран ПВЗ из списка
...
document.addEventListener('mscdek:map:show', e => {
  ...
  console.log(mapBlock, mapUpdParams, ymap, object);
})
```
