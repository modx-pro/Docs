...
| `positionKey` | string | пусто | Синоним `positionName` для старых шаблонов |
...
| `resource` | int | пусто | ID ресурса для таргетинга (override) |
...
| `click_url` | Готовый URL клика (clickout + UTM при включённых настройках) |
| `type` | `image` или `html` |
