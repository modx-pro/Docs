...
Ключ кэша включает **effective context** (`activeContext`): текущий контекст MODX или значение `&context=` из вызова сниппета. Два вызова с разным `context` на одной странице не делят один кэш.
...
При учёте клика `{order_id}` всегда `0`, заказ ещё не создан. `{product_pagetitle}` берётся из `product_id` баннера.

Существующие UTM в URL баннера не перезаписываются. Те же ключи редактируйте во вкладке **Настройки** админки BannerPro.
...
Проверка: [httpbin.org/post](https://httpbin.org/post) или локальный тестовый сервер.
...
Подпись совпадает с `bannerpro_webhook_secret` для клика. POST на URL показа отправляется без ожидания ответа после `OnBannerProImpression`, timeout 2 с.

Тело JSON: `event` (`impression`), `ad_id`, `position_id`, `adposition`, `ip`, `timestamp` (ISO8601), `recorded`, `duplicate`.

Событие `OnBannerProImpression` работает без этой настройки. Webhook нужен только для внешнего HTTP-URL.
