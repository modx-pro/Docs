...
Для legacy-списка без позиций по имени используйте `bannerpro_ads`.

## UTM при клике

В настройках `bannerpro_utm_campaign` и `bannerpro_utm_content` доступны плейсхолдеры `{product_id}`, `{product_pagetitle}`, `{order_id}`. На clickout `{order_id}` всегда `0`.
...
Плагин **BannerProMiniShop3** на `msOnProductPrepare` / `msOnProductsLoad` заполняет на каждом товаре:

| Поле | Формат |
| --- | --- |
| `bannerpro_ads` | Плоский список баннеров (legacy) |
| `bannerpro_positions` | Map `{positionName => [ prepared ad rows ]}` |
