...
```text
Баннер (HTML) → клик → clickout → landing с FormIt
...
URL баннера в админке укажите на ресурс с FormIt. Компонент обернёт HTML в ссылку clickout через `click_url`.

## Шаг 3. UTM (опционально)

Включите UTM в [системных настройках](settings#utm-при-клике) или во вкладке **Настройки** админки. Параметры добавятся к `redirect_url` при клике.
