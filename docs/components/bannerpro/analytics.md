...
| --- | --- | --- |
| MODX | `OnBannerProClick` | Перед перенаправлением после клика |
...
На стенде подключаются `analytics-mock.js` (заглушки `dataLayer`, `_paq`, `ym()`) и `analytics.js`. Кнопки **Симулировать показ/клик** вызывают `bannerproAnalytics.trackImpression/Click`.
...
Для внешней аналитики без клиентских счётчиков настройте [webhook при клике и показе](settings#webhook-при-клике). Сервер отправит POST JSON на ваш URL.
...
| Показы не приходят | `bannerpro_track_impressions`, `BannerProImpression`, баннер в области видимости |
