...
- **Контекст позиции**: при заполненном `context_key` баннеры видны только в этом контексте MODX. Пустое поле: все контексты. Сниппет берёт текущий контекст MODX сам.
- **Таргетинг**: `show_hours`, `target_resource_id`, `target_parent_id`, метки (`&tags=`), лимиты `max_clicks` / `max_impressions`.
- **A/B**: при `sortby=ab` в слоте нужны ровно 2 баннера. Иначе выводятся все подходящие без cookie-деления.

Минимальный вызов с `showLog`:
...
Повторный клик с того же IP за сутки на ту же пару баннер + позиция не увеличивает счётчик. Перенаправление на URL баннера работает.
...
Если у позиции заполнен `context_key`, баннеры этой позиции выводятся только в указанном контексте. Параметр сниппета `&context=` сверяет активный контекст с полем позиции. Подробнее: [Интеграция](integration#контекст-modx).

## Показы не считаются

Проверьте `bannerpro_track_impressions`, плагин **BannerProImpression** и наличие `impression.js` на странице.

Компонент фиксирует показ, когда баннер попадает в область видимости. Повтор с того же IP за сутки не создаёт новую запись.

При `bannerpro_impression_lazy = Да` pixel отправляется после задержки в области видимости, а не сразу при появлении.
...
Параметр `sortby=ab` делит трафик 50/50 только если в позиции **ровно два** активных баннера. Иначе выводятся все подходящие без cookie-деления.

Проверьте:

1. В слоте два баннера, оба проходят фильтры (даты, таргетинг, лимиты).
2. При `sortby=ab` сниппет **не кэширует** HTML (как при `RAND()` и `weighted`). Не оборачивайте вызов во внешний кэш страницы, общий для всех посетителей.
...
4. Редирект не ждёт ответа webhook. Таймаут — 2 с.
...
4. Тело JSON содержит `event: impression`. Подпись в заголовке `X-BannerPro-Signature`, если задан secret.
...
Если вы собираете `.transport.zip` из исходников, перед сборкой выполните `npm run build`.
...
## REST API возвращает 403 write disabled

POST или PATCH при выключенном **`bannerpro_api_write_enabled`**. Включите настройку, если внешней системе нужно создавать или править баннеры. Подробнее: [REST API](development/rest-api#запись-post--patch).

## REST API возвращает 429

Превышен **`bannerpro_api_rate_limit`**. Подождите минуту или увеличьте лимит (`0` = без ограничения).

## REST API: 404 на маршрут с фильтрами

Фильтры передавайте отдельными query-параметрами, не внутри `route`:
...
См. [REST API](development/rest-api#маршрутизация).
