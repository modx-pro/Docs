...
* Настройка `ml_resource_fields` задаёт json массив, в котором указываются поля для переводов:
...
* Настройка `ml_resource_tv_fields` задаёт json массив, в котором перечисляются названия tv полей для перевода:
