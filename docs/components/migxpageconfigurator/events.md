...
По умолчанию пакет сам определяет язык на каждый запрос: берёт список доступных языков, язык по умолчанию и имя cookie из настроек и выставляет `cultureKey`. Это событие позволяет вмешаться в этот момент, не выключая механизм целиком (полный выключатель — настройка [`mpc_set_language_on_request`](settings#лексиконы-и-языки)).
