...
- **MODX Revolution 2** (тестируется на ветке 2.8.x). Для **MODX Revolution 3** есть отдельная версия пакета — её требования ниже.
...
::: tip Требования версии для MODX 3
Версия для **MODX Revolution 3** собирается и работает на:

- **MODX Revolution 3.x** (проверено на 3.2.x).
- **PHP 8.1+**, целевая версия — **8.2** (на ней пакет собирается и тестируется).
- Расширения PHP: **`gd`** или **`imagick`** (обработка картинок), **`intl`** (транслитерация имён скачанных файлов — без него кириллические имена теряются), **`curl`** (скачивание внешних медиа), **`zip`** (экспорт лексиконов).

Сторонние библиотеки (DiDOM, OpenSpout, а также **Glide** + **Intervention Image** + **Flysystem** для обрезки «на лету») поставляются **внутри пакета** через Composer — отдельно ставить не нужно.
:::
...
::: tip Зависимости версии для MODX 3
В версии для **MODX Revolution 3** нужны:

- **pdoTools 3** и **MIGX 3** — те же роли (рендеринг и хранилище полей), но в редакциях под MODX 3.
- **pThumb уже не нужен**: обрезка работает «на лету» через встроенную Glide (см. [Работа с медиа](media)). Достаточно расширения `gd` или `imagick`. Сниппет `mpcThumb` из пакета убран.

После установки пакета его Composer-зависимости (Glide, Intervention Image, Flysystem и др.) уже лежат внутри — доустанавливать ничего не требуется.
:::
...
При **первой установке** пакет показывает окно с базовыми настройками. Введённые значения сразу записываются в системные настройки MODX. При **обновлении** пакета окно не показывается — ваши настройки сохраняются нетронутыми. Позже всё это можно изменить и в разделе «Системные настройки».
...
- **Псевдоним ресурса контактов** (`mpc_contacts_page_alias`) — какой ресурс будет общим хранилищем контактов сайта.
  - **Пусто** — пакет создаёт ресурс с псевдонимом `contacts` на шаблоне «Контакты» (поведение по умолчанию).
  - **Указан псевдоним существующего ресурса** — контакты привязываются к нему: к его шаблону добавляется TV `contacts`, сам шаблон ресурса не меняется (а если шаблона у ресурса нет — создаётся «Контакты» и назначается).
  - **Указан псевдоним несуществующего ресурса** — он создаётся на шаблоне «Контакты».
