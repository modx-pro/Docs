...
- **Фильтр по классу ресурса** — ограничение поиска одним или несколькими `class_key` (только товары, только статьи, и т. п.)
- **Per-class адаптеры** — индексация и рендеринг кастомных моделей (товары miniShop3, комментарии, любые xPDO-объекты) через систему адаптеров
