...
| **classKey** | | Один или несколько `class_key` через запятую — ограничивает поиск выбранными классами ресурсов. Например `MiniShop3\Model\msProduct` — искать только товары. Пустое значение (по умолчанию) — фильтр не применяется, поиск идёт по всем классам. См. раздел «Фильтрация по классу ресурса» |
...
### Только товары miniShop3

```fenom
{'!mSearchForm' | snippet : [
    'autocomplete' => 1,
    'classKey' => 'MiniShop3\\Model\\msProduct',
    'element' => 'msProducts',
    'elementProperties' => [
        'parents' => 0,
        'formatPrices' => 1,
...
Параметр `classKey` отсекает все нетоварные результаты (страницы, статьи, категории) ещё на уровне SQL, оставляя в выдаче только записи с `class_key = MiniShop3\Model\msProduct`. `element` отвечает за обогащение этих записей через сниппет `msProducts`.
...
1. Записывает конфигурацию (`tpl`, `limit`, `ctx`, `classKey`, `element`, `elementProperties`) в кэш MODX под ключом `formId` (TTL 24 часа, partition `msearch/forms`).
...
Сервер принимает от клиента только `query`, `offset` (для основного поиска) и `ctx` (для headless-режима). Все остальные параметры (`tpl`, `limit`, `classKey`, `element`, `elementProperties`) приходят исключительно из кэша по `formId`. Клиент не может подменить имя чанка или подсунуть произвольный сниппет — это закрывает вектор SSTI.
...
## Фильтрация по классу ресурса

Свойство `classKey` ограничивает поиск выбранными классами ресурсов. Применяется на SQL-уровне в таблицах `mse_words` и `mse_intro` — веса, бонусы за точные совпадения и LIKE-fallback считаются уже по сокращённому набору. На больших индексах это заметно быстрее, чем фильтрация после получения результатов.
...
В подсказках появятся только товары miniShop3 — обычные страницы, статьи, категории отсекаются ещё на этапе SELECT'а из индекса.
...
{'!mSearchForm' | snippet : [
    'autocomplete' => 1,
    'classKey' => 'MiniShop3\\Model\\msProduct,Tickets\\Model\\TicketComment',
]}
```

Принимаются и через запятую, и в виде PHP-массива при Fenom-вызове:
...
### Без `classKey` — поиск по всем классам

Если параметр не задан или пустой — фильтр в SQL не добавляется, в выдачу попадают результаты любых классов: ресурсы, товары, кастомные модели, для которых зарегистрирован адаптер. Это поведение по умолчанию.

::: tip Несуществующий class_key
Если указать класс, которого нет в индексе — выдача будет пустой. Не падает, не пишет ошибок, просто `mse_words.class_key IN (?)` не находит совпадений.
:::

### Связь с `element`

`classKey` и `element` решают разные задачи и могут использоваться вместе:

- **`classKey`** — какие классы участвуют в поиске.
- **`element`** — каким сниппетом загружать данные для найденных ID (для одного из адаптеров).

Типичный кейс «только товары и через msProducts»:
...
Без `classKey` `element=msProducts` обогатил бы только товары, а в выдаче остались бы ещё и обычные страницы (через `ResourceAdapter` нативно). `classKey` это пресекает.
...
::: tip Где взять отформатированную цену в чанке
В чанке подсказки доступны два плейсхолдера для цены:

- **`{$price_display}`** — готовая строка, всегда. Безопасно выводить прямо без фильтра. Если включён `formatPrices=1` — будет со значком валюты, иначе — простая числовая форматировка.
- **`{$price}`** — исходное значение. С новым контрактом miniShop3 (PR [#269](https://github.com/modx-pro/MiniShop3/pull/269)) — всегда `float`. На старой miniShop3 с `formatPrices=1` — строка вида `1 500 ₽`.

Если нужно сделать вычисления или сравнения с ценой в чанке — используйте `{$price}` (с новой miniShop3 это число). Если нужно просто вывести цену пользователю — `{$price_display}`.
:::
