...
На вкладке **Настройки раздела** (свойства `properties.tickets`) задают поведение **новых** тикетов в этой секции.

### Дочерние тикеты

| Поле | Описание |
| --- | --- |
| Шаблон дочерних документов | шаблон новых тикетов; если пусто — `tickets_default_template` |
| Формирование URI | шаблон URI: `%y`, `%m`, `%d`, `%alias`, `%id`, `%ext` |
| Показывать в дереве | `show_in_tree` для новых тикетов |
| Скрыть в меню | `hidemenu` для новых тикетов |
| Отключить Jevix | не фильтровать HTML новых тикетов |
| Выполнять теги MODX | разрешить теги MODX в контенте новых тикетов |

Плагин `OnPageNotFound` использует шаблон URI для редиректа с коротких URL секции на тикет. Подробнее: [Фронтенд](/components/tickets/frontend).

### Рейтинги

Очки рейтинга автора за действия в разделе: создание тикета, комментарий, просмотр, голос, избранное. Значения попадают в плейсхолдер `ratings` в `getComments` и в мета тикета.

Полный список ключей политик: [Политики и права](permissions).
