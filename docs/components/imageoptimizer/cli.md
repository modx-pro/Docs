...
| Retry failed | нет прямого CLI; **Повторить выбранные** в UI или connector `queue/retry` |
| Reset stuck | автоматически в cron; в UI — **Сбросить зависшие** |

`queue/process`, cron и кнопка **Обработать очередь** используют один lock. Параллельный запуск вернёт `worker_busy`. UI повторяет запрос до 3 раз с паузой 2 с.
