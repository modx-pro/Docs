...
1. **Обработать очередь** в админке — батчи до `pending = 0`, можно **Остановить** ([Админка](manager))
...
Обновите пакет: worker создаёт `core/cache/imageoptimizer/` перед копированием. Либо вручную:
...
Обновите до 1.0.4+. Старый HTML-кэш удалите: **Очистить кэш** MODX или **Очистить варианты** в админке. Подробнее: [Авто-inject и picture](frontend#utf-8-i-kirillica).
