...
Возвращает ID элементов списка избранного. Данные берутся из БД по `user_id` или `session_id` (гости при включённой настройке `guest_db_enabled`). Если БД пуста — из `cookie`.
...
| **return** | `str` — строка ID через запятую; `data` — массив id (для вызова через `$modx->runSnippet()` или в связке с **toPlaceholder**) | `str` |
...
- Пустой список: строка `-0` (для проверки `[[+name:is="-0":then=...]]`); при `return=data` — пустой массив
- Непустой: строка ID через запятую; при `return=data` — массив int
