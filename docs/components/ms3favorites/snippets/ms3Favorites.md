...
| **page** | Номер страницы. Работает и без `pdoPage`: сдвиг выборки `(page − 1) × limit` | 1 |
| **offset** | Смещение выборки. Явный `offset` имеет приоритет над `page`; не задан — считается из `page`. При пагинации смещение передаёт pdoPage | — |
