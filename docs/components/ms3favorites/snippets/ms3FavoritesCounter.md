...
Без `data-resource-type` (или со значением `all`) клиентский счётчик показывает сумму по всем типам ресурсов. С атрибутом, например `data-resource-type="products"`, — только по этому типу.
