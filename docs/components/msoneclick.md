...
В miniShop2 добавляются метод доставки и метод оплаты «Быстрый заказ» и назначаются для создания заказов через модальное окно. В настройка приложения можно изменить эти способы.
...
|-----------------------|------------------------------|------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **required_fields**   | `receiver,email,phone`       | Обязательные поля для заполнения пользователем |                                                                                                                                                                                                          |
...
| **mask_phone**        | `true`                       | Включить маску телефонного номера              | На поле с телефоном будет добавлена обязательная маска +7 (999) 999 9999                                                                                                                                 |
| **mask_phone_format** | `+9 (999) 999-9999`          | Маска телефона                                 | Формат ввода должен: +9 (999) 999-9999                                                                                                                                                                   |
| **framework**         | `default`                    | Подключить framework                           | По умолчанию default будет подключен свой скрипт для запуска модального окна. Можно указать default,bootstrap,semantic,materialize,uIkit для этих фреймворков определены функции запуска модального окна |
...
| **email_method_mail**    |                                                      | E-mail, куда будет уходить письмо, если использовать метод MAIL. Если оставить пустым, то будут использоваться настройки ms2_email_manager или emailsender                           |
| **tplBtn**               | `tpl.msOneClick.btn`                                 | Чанк оформления кнопки «быстрый заказ»                                                                                                                                               |
| **tplModal**             | `tpl.msOneClick.modal`                               | Чанк оформления модального окна                                                                                                                                                      |
...
| Параметр                  | Описание                                 |
|---------------------------|------------------------------------------|
| **msoneclick_load**       | Срабатываем после получение данных ajax  |
...
#### Переопределение методов для модального окна
...
Пользователи могут выбрать опции находясь на странице. При запуске модального окна опции не будут выбраны так как они запоминаются только на странице.

Для того чтобы опции автоматически были выбраны в модальном окне, необходимо добавить в форму где выбираются опции класс с id товара:
...
```js
$(document).on('msoneclick_after_init', function (e, data) {
  // После запуска модального окна ищется форма на странице
  var form = $('.msoptionsprice-product-' + msOneClick.Product.product_id)
  if (form.length) {
    // Если форма найден отправляется запрос для обновления данных в форме модального окна
