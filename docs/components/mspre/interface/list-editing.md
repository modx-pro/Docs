...
## Привязка к категории и Открытие доступа для шаблона

Некоторые опции и Тв параметры могу быть не привязаны к категории или закрыт доступ к шаблону.
Для этого реализован специальных механизм которые перед тем как запустить модальное окно для редактирования, проверяет
...
Запускается модельные окна, с типом поля для ввода согласно **Типу Характеристики**
