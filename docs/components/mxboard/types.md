...
Плюс всегда доступны `assignee` (исполнитель), `priority`, `plan_hours` (плановая трудоёмкость в часах, `0` — не оценивали, см. [План и факт](permissions#plan-i-fakt)) и `meta` (произвольные данные интегратора).

::: info Свободный текст — в комментариях
Начиная с mxBoard 3.0 отдельного поля `tor` нет. Структурированная постановка хранится в полях типа задачи, а свободный Markdown, уточнения и обсуждение — в комментариях карточки. При обновлении с 2.x старые непустые значения `tor` автоматически переносятся туда без изменения текста.
:::
