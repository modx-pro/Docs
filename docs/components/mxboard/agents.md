...
| `task_get` | карточка: поля, родитель, подзадачи, состояние и срез комментариев (параметр `comments`) |
...
| `task_delete` | удалить карточку безвозвратно (автор/менеджер); закрытую — нельзя |
...
`task_delete` удаляет карточку вместе с её комментариями, журналом, вложениями и уведомлениями; подзадачи при этом **открепляются, а не удаляются**. Факт удаления пишется в журнал, автор и исполнитель получают уведомление, а если карточка стояла первой в очереди — очередь двигается дальше. Инструмент для ошибочно созданных карточек, а не для закрытия работы: отработанную задачу переводят в финальную стадию через `task_move`.

### Лента комментариев по запросу

Самая тяжёлая часть карточки — переписка: на живой карточке она занимала 97% объёма ответа (41 тыс. символов против 3,6 тыс. у остального) и возвращалась в контекст агента при **каждом** чтении. Поэтому `task_get` по умолчанию отдаёт шапку, поля, подзадачи, **последний** комментарий и строку-счётчик вида «Комментарии: всего 13, показано 1, id последнего 2924».

Остальное берётся явно, параметром `comments`:

| Значение | Что вернётся |
| --- | --- |
| `last:1` | по умолчанию — последний комментарий |
| `last:N` | последние N |
| `since:<id>` | только появившиеся после указанного id — «догнать непрочитанное» |
| `all` | вся лента, как раньше |
| `0` | без ленты вовсе, одни счётчики |

Id последнего комментария печатается всегда — его и передают в `since:` на следующем чтении.

Ещё в шапке есть строка **«Состояние»**: в какой стадии карточка, с какого момента, кто перевёл и чей ход. Она считается из журнала, поэтому понять, ждут ли действия от тебя, можно не читая переписку.
...
| GET | `/tasks/{id\|num}` | карточка целиком; `?comments=last:N\|since:<id>\|all\|0` режет ленту, в ответе `comments_total`, `comments_shown`, `comments_last_id` |
...
Дефолт по комментариям у каналов **разный и намеренно**: REST отдаёт всю ленту, как и до 3.3.0 — внешние интеграции и скрипты, которые её парсят, ломать нельзя; MCP отдаёт последний комментарий, потому что там на другом конце агент с оплачиваемым контекстом. Нужна экономия в REST — попросите её явно параметром `comments`.
...
Участники приходят и числами, и логинами: `author_id` / `assignee_id` — id пользователей MODX, `author` / `assignee` — их `username` (с 3.2.0-pl), `actor` — логин того, кто совершил действие. Логины нужны, чтобы разбирать упоминания в тексте комментария: если бот адресует комментарий по `@логин`, сопоставить упоминание с участником карточки по одним числовым id нечем.
