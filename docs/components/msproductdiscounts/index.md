...
16. Журнал действий менеджеров со скидками и промокодами.

## Разделение доступа в менеджере

Начиная с версии `2.7.0-rc`, установщик создаёт шаблон политик
`msproductdiscountsTemplate` и две готовые политики:

| Политика | Что разрешает |
|---|---|
| `msproductdiscountsFull` | Управление скидками и промокодами |
| `msproductdiscountsPromocodes` | Управление только промокодами |

Назначьте нужную политику группе пользователей в разделе управления доступом вместе с
обычной политикой входа в менеджер. Минимальной ролью в записи ACL можно разделить доступ
внутри одной группы: например, полную политику выдать роли Super User, а политику только
для промокодов — роли Member.

MODX объединяет разрешения всех подходящих записей ACL. Поэтому не назначайте пользователю
другую политику, содержащую разрешение `mspd_discounts`, если он должен работать только с
промокодами. Пользователь `sudo` по правилам MODX сохраняет полный доступ.
...
| **mspd_show_discount_info**    | `Да`                                | выводить ли описание акции (шаблон `mspd_info_tpl`) у скидок, где настройка `Описание акции` оставлена в значении `По умолчанию`                                                                      |
...
Начиная с версии `2.4.0-rc` компонент ведёт журнал того, кто и когда менял скидки и промокоды.
Записываются семь действий: создание, изменение, удаление и дублирование скидки, а также
создание, изменение и удаление промокода.

Журнал ведётся через компонент [mxLogger](https://modstore.pro/packages/users/mxlogger) — записи
видны в его разделе в админке. Если `mxLogger` на сайте не установлен, журнал просто не ведётся:
на работу скидок и промокодов это никак не влияет. Отдельной настройки у журнала нет, режим
отладки `mspd_debug` для него включать не нужно.

Что попадает в запись:

| Действие | Что сохраняется |
|---|---|
| Любое | пользователь (id и логин), время, действие, объект: id и название скидки или код промокода |
| Создание, дублирование | все значения объекта; при дублировании ещё и исходная скидка |
| Изменение | только изменённые поля — парами «было / стало», у промокода так же смена привязанных скидок |
| Удаление | полное состояние удалённого объекта |

Значения полей сохраняются целиком, поэтому по журналу можно восстановить прежние настройки
скидки.

Как найти нужные записи в `mxLogger`:

* все действия со скидками и промокодами — фильтр по тэгам `msproductdiscounts` и `audit`;
* только скидки или только промокоды — добавьте тэг `discount` или `promocode`;
* конкретное действие — тэг `create`, `update`, `remove` или `duplicate`;
* вся история одной скидки или одного промокода — процесс `mspd_discount_<id>` или
  `mspd_promocode_<id>`, где `<id>` — идентификатор объекта.

::: warning
Записи хранятся столько, сколько задано в настройках `mxLogger` (`mxlogger.log_lifetime`,
по умолчанию — неделя). Если журнал действий нужен на больший срок, увеличьте это значение.
:::
...
* `[data-mspd-prop="price_str"]` - блок вывода отформатированной цены товара, если нужна неотформатированная цена укажите в качестве значения `price`.
* `[data-mspd-prop="old_price_str"]` - блок вывода отформатированной старой цены товара, если нужна неотформатированная старая цена укажите в качестве значения `old_price`.
...
* С версии `2.1.19-rc` включаемых Опций и Свойств может быть несколько, и проверяются все: условия группируются по ключу — внутри одного ключа достаточно совпадения любого значения («ИЛИ»), а условия по разным ключам должны выполняться одновременно («И»). Например, два условия `color = red` и `color = green` разрешат скидку и красным, и зелёным товарам, а пара `color = red` и `width = 10` — только красным товарам с шириной `10`. В более ранних версиях проверялось только первое включаемое условие. Это позволяет, в частности, ограничить скидку несколькими контекстами через свойство `context_key`.
...
У каждой скидки есть также поле «Тип применения» (учитывается при `mspd_many_behaviour` = `3`):

* «На общих условиях» — скидка участвует в общем порядке применения;
* «Только эту скидку» — если товару подходит такая скидка, к нему применяется только она;
* «Все кроме этой» — с версии `2.1.20-rc` такая скидка применяется после остальных и только к товарам, **фактически не получившим** скидку в текущем пересчёте корзины (в более ранних версиях она отбрасывалась при наличии у товара любых других подходящих по условиям скидок, даже если те не дали ему выгоды). Удобно для промокодов: код даёт выгоду только товарам, оставшимся без акции, и не складывается с ней на одном товаре. В превью карточек такая скидка не стекуется с другими.
* «Суммируется с другими» — с версии `2.1.22-rc` такая скидка не участвует в эксклюзивном отсечении: если товару подходит скидка «Только эту скидку», суммируемая всё равно применяется поверх неё. Порядок применения в корзине: обычные скидки → «Все кроме этой» → «Суммируется с другими». Благодаря такому порядку суммируемая скидка не отбирает у «Все кроме этой» её товары — товар по-прежнему считается «без скидки», если получил только суммируемую. Математика — каскад от текущей цены: `1000` руб. при основной скидке `10%` и суммируемой `5%` даёт `855` руб., а не `850`.

## Показ скидки в каталоге и карточке товара

С версии `2.2.0-rc` у каждой скидки есть три настройки показа: `Показ в каталоге`, `Показ в карточке` и `Описание акции`. Каждая принимает одно из трёх значений:

* `По умолчанию` — поведение определяют системные настройки, как и раньше: `mspd_show_for_all` и `mspd_check_common_conditions_on_show` для цены, `mspd_show_discount_info` для описания акции;
* `Показывать` — скидка участвует в расчёте цены для показа **независимо от корзины**: цена со скидкой и зачёркнутая старая цена видны, даже если корзина пуста, а общие условия скидки (минимальная сумма, количество) ещё не выполнены;
* `Не показывать` — скидка не участвует в показе совсем, в том числе при `mspd_show_for_all` = `Да`.

До появления этих настроек показом управляли только системные настройки, то есть включить показ ради одной акции и не включать его для остальных было нельзя. Типичный сценарий: акция «Товары недели» показывает в каталоге старую и новую цену, а скидка «−20% на вторую единицу» в списке товаров не показывается, потому что её цена зависит от состава корзины.

Настройки влияют **только на показ**. Корзина по-прежнему считает фактические скидки по общим правилам, поэтому у скидки, скрытой в каталоге, цена в корзине может отличаться от показанной в списке товаров.

Каталог и карточка товара различаются атрибутом `data-mspd-scope`: значение `product` на обёртке товара или на любом его предке означает карточку товара, всё остальное (и отсутствие атрибута) — каталог. Шаблоны без этого атрибута работают как раньше — запрос считается запросом из каталога.

```html
<!-- страница товара: сюда попадают настройки «Показ в карточке» -->
