...
6. Теперь нам нужна форма для ввода промокода. Я использовал компонент `AjaxFormitLogin` и вызвал форму на странице корзины. Вызов выглядит так
    ...
    Чанк формы `promocodeForm.tpl` имеет следующее содержимое
...
8. Кроме того после применения промокода, нужно изменять остаток и обновлять корзину. Это мы сделаем в сниппете-хуке `applyPromocode` с таким кодом
