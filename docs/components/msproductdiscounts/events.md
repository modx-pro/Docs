...
#### mspd:cart:changed - состав корзины изменился на сервере

Событие возникает, когда сервер сообщил об изменении состава корзины: компонент добавил или снял подарок. **Может быть отменено.**

Начиная с версии 2.6.0 компонент не изменяет разметку корзины сам: он лишь сообщает событием, что состав изменился, и передаёт новый состав с итогами. Тот, кто рисует корзину, обязан перерисовать её и **отменить событие** — отмена служит признаком «корзина перерисована». Если событие никто не отменил, перерисовать некому и компонент перезагружает страницу.

Имя события настраивается параметром **cartChangedEvent** модуля Cart, значение по умолчанию — `mspd:cart:changed`. Компонент [msAltCart](/components/msaltcart/) версии 1.2.0 и выше слушает это событие из коробки: рисует корзину и отменяет событие сам.

Доступные параметры:

* **products** - состав корзины: данные товаров по ключу товара в корзине, у каждого в **html** лежит разметка ряда по имени корзины.
* **totals** - итоги корзины (`total_count`, `total_cost` и прочие ключи `total_*`).
* **data** - полный ответ сервера.
* **object** - экземпляр класса Cart.

::: details Пример использования

```js:line-numbers
document.addEventListener('mspd:cart:changed', e => {
  const {products, totals} = e.detail;

  renderMyCart(products, totals); // рисуем корзину своими средствами

  e.preventDefault(); // без отмены компонент перезагрузит страницу
});
```

:::

#### mspd:gift:manage - подарок добавлен или снят

Событие возникает перед реакцией на изменение состава корзины подарками. **Может быть отменено**: отмена останавливает дальнейшую обработку — событие `mspd:cart:changed` не возникает и страница не перезагружается, всё управление изменившейся корзиной переходит к подписчику.

Доступные параметры:

* **data** - полный ответ сервера, включая **products** и **totals**.
* **object** - экземпляр класса Cart.

::: details Пример использования

```js:line-numbers
document.addEventListener('mspd:gift:manage', e => {
  const {data} = e.detail;
  console.log(data.products, data.totals);
});
```
