...
Существенный прирост скорости даёт только при первом запуске, дальше Wayfinder не особо уступает, благодаря грамотному кэшированию.
...
| **&showLog**            | `0`                  | Лог в плейсхолдер (с 3.0), не в HTML. Нужна сессия mgr. [Подробнее](../general-properties#showlog). |
...
| **&tplInner**           | Чанк оформления всего блока подпунктов меню. Если пуст - будет использовать **&tplOuter**. Например: `@INLINE <ul class="submenu [[+classnames]]">[[+wrapper]]</ul>`. С **pdoTools 3.1.0+ (MODX 3)** также доступен `[[+children]]` (число потомков). |
...
## Плейсхолдеры строки {#row-placeholders}

::: tip Доступно в pdoTools 3.1.0+ (MODX 3)
Булевы флаги как `0` / `1` в чанках пунктов меню. Уже заданные чанки `tpl*` по-прежнему работают; можно ветвиться по этим плейсхолдерам вместо CSS-классов или вместе с ними.
:::

| Плейсхолдер | Смысл |
| --- | --- |
| `isFirst` | Первый пункт на уровне (`idx == 1`) |
| `isLast` | Последний пункт на уровне |
| `isActive` | Текущий ресурс (`hereId`) |
| `hasChildren` / `hasChilds` | Есть вложенные потомки (второй ключ для старых шаблонов) |
| `isHere` | Пункт на пути к текущему ресурсу |
| `isStart` | Корневой узел при включённом **&displayStart** |
| `isCategory` | Родитель с потомками и пустым шаблоном или `category` в `link_attributes` |
| `isInner` | Вложенный уровень (`level > 1`) |
