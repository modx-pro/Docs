...
У каждого слота пагинации (`tplPage`, `tplPageActive`, `tplPageSkip`, …) флаги `0` / `1`. Отдельные чанки `tpl*` по-прежнему имеют приоритет, если заданы.
:::

| Плейсхолдер | Смысл |
| --- | --- |
| `isFirst` | Номер страницы равен 1 |
| `isLast` | Номер страницы равен числу страниц |
| `isActive` | Текущая страница (не слот пропуска) |
| `isSkip` | Слот многоточия в продвинутом режиме |

### Свойства сниппета в шаблонах пагинации

::: tip Доступно в pdoTools 3.1.0+ (MODX 3)
При рендере **&tplPageWrapper** (и связанных чанков пагинации) pdoPage подмешивает свойства вызова сниппета в плейсхолдеры. В этих шаблонах доступны ваш `&limit`, `&element` и произвольные ключи из вызова.
:::
