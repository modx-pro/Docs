...
У каждой крошки флаги `0` / `1`. Уже заданные **&tpl** / **&tplCurrent** / **&tplHome** по-прежнему работают.
:::

| Плейсхолдер | Смысл |
| --- | --- |
| `isFirst` | Первая крошка в списке |
| `isLast` | Последняя крошка в списке |
| `isActive` | Крошка указывает на текущую страницу |
| `isHome` | Крошка указывает на стартовый ресурс сайта |
