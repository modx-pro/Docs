...
- **pdotools_cache_path** - каталог файлового кэша pdoTools/Fenom. По умолчанию `{core_path}cache/pdotools`. **pdoTools 3.1.0+ (MODX 3)**.

::: tip Доступно в pdoTools 3.1.0+ (MODX 3)
Ошибки компиляции и выполнения Fenom называют чанк или ресурс, дают короткий фрагмент исходника; ключ кэша по-прежнему хеш контента.
:::
