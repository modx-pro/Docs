...
Если mxLogger у пользователя точно установлен (он в зависимостях вашего пакета),
проще всего — единый фасад `$modx->mxl` (с версии 1.2.0), одинаково в обеих версиях:

```php
$modx->mxl->info('mypackage', 'Импорт начат', ['rows' => $count]);
// ... ваша логика ...
$modx->mxl->error('mypackage', 'Импорт упал', ['row' => $i, 'error' => $e->getMessage()]);
```

Получить сам сервис явно (например, для совместимости с версиями ниже 1.2.0):
