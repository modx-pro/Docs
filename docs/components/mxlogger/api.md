...
С версии **1.2.0** в обеих версиях есть короткий фасад **`$modx->mxl`** — сервис
доступен сразу из любого сниппета, плагина или чанка, получать его не нужно:

```php
$modx->mxl->info('purchase', 'Корзина создана', ['cart_id' => $id]);
```

Это рекомендуемый способ: код одинаков и в MODX 2, и в MODX 3.

Если фасад недоступен (версия ниже 1.2.0 или очень ранний этап загрузки), сервис
получают явно — это единственное, что различается между версиями; дальше API
полностью одинаков.
...
В примерах ниже `$mxl` — это `$modx->mxl` (или явно полученный сервис); вызовы
идентичны.
