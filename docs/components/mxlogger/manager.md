...
- **Экспорт** (с версии 1.2.0) — кнопка «Экспорт» в тулбаре: выгружает записи с
  учётом текущих фильтров (тех же, что видны в гриде) в **Markdown** (`.md`) или
  **текст** (`.txt`). Выгрузка включает `context` и `trace`; порядок
  хронологический. Без фильтров — весь журнал.

Коннектор грида — `assets/components/mxlogger/connector.php`. Процессоры в MODX 3 —
`MxLogger\Processors\Mgr\Log\{GetList,Get,GetTags,Clear,Remove}`. Экспорт отдаётся
отдельным потоковым эндпоинтом `assets/components/mxlogger/export.php` (файл-вложение,
а не JSON; доступ только авторизованному менеджеру).
