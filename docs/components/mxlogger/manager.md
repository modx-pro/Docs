...
Менеджер → **Компоненты → mxLogger**. Грид на Vue 3 + PrimeVue.

- **Фильтры** (применяются автоматически, без кнопки): тэги (мультиселект с
  чекбоксами; несколько выбранных → совпадение по всем), уровень, Process UID,
...
Процессоры грида: `MxLogger\Processors\Mgr\Log\{GetList,Get,GetTags,Clear,Remove}`,
коннектор — `assets/components/mxlogger/connector.php`.
