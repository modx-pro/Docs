...
| `limit` | настройка `mfilter.default_limit` | Количество элементов на странице. Без параметра берётся значение системной настройки (по умолчанию `20`); заданный здесь `limit` её перекрывает |
| `offset` | `0` | Начальное смещение |
| `searchVar` | `mse_query` | Имя параметра запроса с поисковой фразой. Работает в режиме `&element`. Не путать с `queryVar` — это параметр самого элемента mSearch, `mFilter` его не читает и пробрасывает элементу как есть |
| `sortby` | `pagetitle` | Поле сортировки |
| `sortdir` | `ASC` | Направление сортировки (ASC, DESC) |
| `where` | — | Дополнительные условия: массив в формате xPDO (из Fenom) или JSON-строка (из MODX-тегов). В native-режиме ключи вида `Data.*` сами подключают таблицу `msProductData` |
| `select` | — | Поля для выборки (JSON) |

::: warning Не передавайте в `where` данные из запроса
Значения условий экранируются, а **ключи** — `Data.price:>` — попадают в SQL как имена полей и операторы. Параметр рассчитан на значения, заданные разработчиком. Если условие строится из того, что прислал посетитель, сверяйте поле с белым списком до вызова сниппета.
:::

::: tip `Data.*` без MiniShop3
В native-режиме, если MiniShop3 не установлен, условие с ключами `Data.*` пропускается **целиком** — вместе с остальными ключами того же `where` — с записью в журнал ошибок MODX. Выборка при этом не падает, но и не ограничивается. В режиме `&element` `where` передаётся элементу как есть, и обработка остаётся за ним.
:::
...
| Параметр | По умолчанию | Описание |
|----------|--------------|----------|
| `useFilters` | `true` | Применять активные фильтры из URL/POST. Установите `false`, чтобы получить полную выборку без учёта фильтрации |
