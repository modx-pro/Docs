...
| Рецепт | Описание |
|--------|----------|
| [Фильтр выпадающим списком](select-filter) | Вывод значений фильтра в `<select>` вместо чекбоксов |
...
Включена по умолчанию (настройка `mfilter.auto_submit`) и срабатывает на любом поле формы — чекбоксах, радио и `<select>`. Дополнительный код не нужен.

Свой обработчик `change` поверх штатного вредит в обоих режимах: при включённой автоотправке может уйти повторный запрос, а при `&autoSubmit=`0`` форма начнёт отправляться на каждое изменение — ровно то, что выключали. Как вывести фильтр списком — в рецепте [Фильтр выпадающим списком](select-filter).
