...
| `resource_ids` | Загрузка страницы, `mFilter` вызван с `&element` |
| `parents` | Загрузка страницы, `mFilter` вызван без `&element` — родители товаров |

Проверять нужно в этом порядке. Если читать только `parents`, в режиме с `&element` фильтр останется без значений. Если не читать `filtered_ids`, число товаров не будет меняться после выбора других фильтров.
