...
| Пустые значения не блокируются после фильтрации | `name` с `[]`. У селекта с `multiple` это ограничение JS — см. «Несколько значений» |
| Счётчик у заголовка фильтра не считает выбор в селекте | То же: `name` с `[]` |
| После «Назад» в селекте с `multiple` выбрано одно значение из нескольких | `multiple` без `[]` в имени |
| Выбор в селекте не отменить в самом селекте | В чанке обёртки нет опции селекта «Все варианты» |
