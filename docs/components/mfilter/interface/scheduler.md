...
**Если пересборка уже идёт**, запуск не выполняется, а встаёт в очередь на две минуты позже — и так до тех пор, пока индекс не освободится. В логе это видно строкой `Facet index rebuild postponed`. Работа не теряется: дождавшись очереди, запуск прочитает каталог заново. Подробнее: [Обслуживание → Две пересборки одновременно не идут](maintenance#две-пересборки-одновременно-не-идут).
...
**Если индексация TV или генерация алиасов уже идёт**, запуск встаёт в очередь на две минуты позже — так же, как пересборка индекса фасетов. В логе это строка `Slug reindex postponed`.
