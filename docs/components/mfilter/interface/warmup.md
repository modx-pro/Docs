...
Прогрев `baseIds` остаётся в продукте только для AJAX-режима получения базовых списков ID. Для основной фильтрации (значения фильтров и suggestion'ы) теперь используется [индекс фасетов](maintenance) — он быстрее, не требует ручной настройки и не накапливает stale-данные.

Не отключайте прогрев, если у вас работает AJAX-фильтрация — он ещё нужен. После стабилизации индекса фасетов будет упрощён или удалён.

В админке вкладка перенесена в сворачиваемый блок на странице **Обслуживание**.
:::

Вкладка **Прогрев кэша** позволяет управлять конфигурациями прогрева — заранее вычислять и кэшировать данные фильтров.
...
::: tip
С версии 1.4.0 пункты 2 и 3 закрыты [индексом фасетов](maintenance) — прогрев нужен только для пункта 1 (baseIds в AJAX-режиме).
:::
