...
::: warning Сейчас объявление не действует
Вид значений определяется по ключу фильтра, без типа из набора. Тип по ключу ищется через `supports()`, а он у части встроенных типов принимает любой ключ, поэтому для любого ключа находится `number` и вид `VALUE_KIND_SCALAR`. Из-за этого без слагов остаются и значения встроенного `parents`: адрес `/catalog/parent--15/`.
:::
