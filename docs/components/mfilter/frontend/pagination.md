...
`data-mfilter-hash="{$hash}"` обязателен: по нему AJAX находит сохранённую конфигурацию вызова. Переменная `$hash` существует только внутри чанка — переносить эту обёртку в шаблон ресурса нельзя, подробности в [Плейсхолдерах](placeholders.md#pls-v-shablone-i-v-chanke-eto-raznye-massivy).
