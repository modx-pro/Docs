...
JS-часть находит элементы двумя способами: по `data-`атрибутам и по классам `mfilter-*`. Оба — часть контракта, а не оформление. Переписывая чанки под свой дизайн, сохраняйте их: свои классы можно добавлять рядом. Третья часть контракта — [экранирование значений](#ekranirovanie-znacheniy).
...
<div class="mfilter-wrapper"
     ...
     data-mfilter-keys="{$filterKeys | esc}"
     data-base-url="/{$_modx->resource.uri | esc}"
...
Всё, что приходит из данных, выводите через модификатор `esc`: `{$pagetitle | esc}`. Без него фигурная скобка в названии товара или в значении опции роняет страницу: там, где разметка проходит через Fenom, она перестаёт компилироваться, и в ответ уходит пустое тело.

| Экранировать | Оставить как есть |
|---|---|
| Название, описание, артикул, производителя, значения опций | Числа: `{$total}`, `{$price}`, `{$count}` |
| Ключи фильтров, слаги, подписи значений | Вложенную разметку: `{$rows}`, `{$items}`, `{$pagination}`, `{$formAttrs}` |
| Адреса: `{$baseUrl}`, `{$thumb}`, результат `\| url` | Строки лексикона |

Экранировать нужно и в значениях атрибутов, и в тексте. Браузер вернёт скобку на место, а Fenom её уже не увидит.

::: warning Своя копия чанка исправления не получит
Обновление компонента не перезаписывает чанки, которые уже есть на сайте. В поставке значения экранированы, а в свою копию `| esc` придётся дописать самому.
:::
