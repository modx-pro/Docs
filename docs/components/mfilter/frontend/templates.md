...
Переменной `$hash` в чанке `mFilterForm` не существует: форма рендерится раньше сниппета `mFilter`, и плейсхолдер на тот момент ещё не выставлен. Атрибут нужен в обёртке результатов (`tplOuter` сниппета `mFilter`) — см. [Контракт разметки](#контракт-разметки).
:::
