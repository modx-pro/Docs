...
После записи проекции компонент сбрасывает кэш ресурса товара, чтобы витрина сразу показала новое значение.
...
Тот же пересчёт делает endpoint `POST /api/mgr/ms3remains/sync/rebuild` и сервисный метод `rebuildBatch(array $productIds)`.
