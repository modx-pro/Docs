...
## Шаг 5: Плашка на странице заказа (опционально)

Если хотите показывать визуальный блок «Скидка на первый заказ», добавьте в шаблон страницы оформления заказа:
...
Плагин сам подключит CSS/JS компонента на веб-страницах. Для гостей плашка уточняется после ввода email/phone через endpoint eligibility.
