...
Проверить все три сниппета на одной странице без товаров в корзине можно по инструкции **FRONTEND_INTEGRATION** в поставке: демо-страница в проекте MODX (типичный путь `core/elements/templates/demo/mxdadata_test.tpl`), префикс `id` вроде `mxdadata_test_…` и явный `suggestions` / **`suggestionsChunk`** → **`chunk.mxdadata.demoFormSugTest`**. Либо вручную вставьте сниппет с непустым `suggestions` / `suggestionsChunk` (без конфига в браузере будет пустой массив подсказок).
