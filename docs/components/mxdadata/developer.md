...
из корня пакета (в `package.json` — скрипты `npm run test:smoke` и `npm test`).
...
Переустановка пакета — по стандартной процедуре MODX. При смене схемы смотрите резолверы в `_build/resolvers/`.
