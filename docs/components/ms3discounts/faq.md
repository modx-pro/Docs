...
У правила выключен флаг `show_in_catalog`. Сниппет `ms3discountsGetDiscount` в режиме `prepareSnippet` проверяет только его. На карточке товара проверяется флаг `show_in_product`.
...
Если во входящих свойствах одновременно есть `id` товара и строковый ключ `pagetitle`, это строка `msProducts`. Сниппет дописывает поля скидки и возвращает массив без вызова шаблонизатора. ID товара не затирается, а ID акции сохраняется в поле `sale_id`.
