...
4. На карточке товара вызовите `ms3discountsGetDiscount`. В каталоге цены пересчитает плагин, а бейдж выведите вызовом сниппета в чанке строки `msProducts`.
...
Флаги витрины: `show_in_catalog` (подборки акций и `ms3discountsBuyNow`), `show_in_product` (карточка товара и бейдж в строке каталога), `show_description` (вывод описания акции). Флаг `activation_required` откладывает применение правила до вызова программного активатора.
