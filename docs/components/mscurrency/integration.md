...
| AJAX без reload | [AJAX-переключение](#ajax-переключение) |
...
| GeoIP | [Автовалюта по GeoIP](#автовалюта-по-geoip) |
...
Стили компактного вида — классы `.msc-currency-switcher--dual` и `.msc-currency-switcher--compact-view` в `default.min.css`.

## AJAX-переключение

Настройка **`mscurrency_ajax_switch`** (по умолчанию выкл.).

| Режим | `web/currency/set` | Витрина |
|--------|-------------------|---------|
| AJAX **выкл.** | `{ "success": true }` без тяжёлого `object` | reload / `mfilter.reload()` |
| AJAX **вкл.** | полный `object`: `selectedId`, `placeholders`, `prices`, `currencies` | обновление DOM без reload |
| Невалидная / неактивная валюта | `{ "success": false, "message": … }` | валюта в сессии не меняется |

При **включённом** AJAX:

1. POST `web/currency/set` с `id` и `pids[]` (ID из `[data-msc-price][data-msc-pid]` на странице).
2. JS обновляет `[data-msc-price]`, элементы `[data-msc-ph="msc.code"]` (и др.), переключатель.
3. События `mscurrency:placeholders`, `mscOnToggleCurrency` → `mfilter.reload()`.

Разметка цен: при AJAX и `pid > 0` сниппет `msCurrencyPrice` выводит `<span data-msc-price data-msc-pid="ID">…</span>`. Без `pid` обёртка не ставится — цена не обновится без reload.

Плейсхолдеры в разметке (опционально):

```html
<span data-msc-ph="msc.symbol_left">[[!+msc.symbol_left]]</span>
```

`window.mscConfig.ajaxSwitch` — флаг с сервера. API: `window.mscCurrency.isAjaxSwitchEnabled()`, `normalizeSetResponse()`. Для mFilter подключите `mfilter-currency.js` после `default.min.js`.

Подробнее: [Системные настройки](settings#витрина), [Сниппет msCurrencyPrice](snippets/msCurrencyPrice#ajax-разметка).
...
### Округление цен по валюте

В админке у каждой валюты поле **Округление цены** (`price_rounding`): `none`, `round`, `ceil`, `floor`, `to_99` (психологическая цена, например 100.12 → 100.99). Применяется при конвертации на витрине (`PriceConversionService`).

## Автовалюта по GeoIP

Если покупатель ещё не выбирал валюту сам, msCurrency может подставить её по стране визита.

1. Включите плагин **`mscurrency_detect`**.
2. `mscurrency_geoip_enabled` = Да.
3. `mscurrency_geoip_respect_manual` = Да — GeoIP не перебьёт валюту после ручного переключения.

Страна берётся из заголовков CDN/веб-сервера: `HTTP_CF_IPCOUNTRY`, `GEOIP_COUNTRY_CODE` и аналоги. Код страны сопоставляется с валютой (RU→RUB, DE→EUR, US→USD, PL→PLN, …). Если страна неизвестна или валюта неактивна — берётся `mscurrency_selected_currency_default` или базовая валюта.

**Сессия:** `msc_currency_id` — выбранная валюта; `msc_currency_manual` — флаг ручного выбора.

**Проверка на стенде:** GET-параметры `msc_geo_reset=1` (очистка сессии) и `msc_geo_country=DE` (имитация страны, приоритет выше заголовков).
