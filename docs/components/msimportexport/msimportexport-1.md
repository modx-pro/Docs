...
Файл импорта должен содержать как минимум 2 обязательных поля pagetitle и поле которое указано в настройках импорта в опции **“Ключ товара, по которому определяется его уникальность”** (по умолчанию “article”)
...
импорта опция **«Базовая директория для изображений»)** так и http/https ссылкой на сторонний ресурс (при использовании http/https ссылки импорт происходит медленнее). Через
...
**alias** — данное поле, как правило, импортировать не нужно, т. к. оно автоматически создаётся MODX на основании значения из pagetitle. Отсюда следует, что pagetitle должно быть уникальным относительно категории.
...
**Пример импорта цвета для товара:** `Зелёный|Красный`.

### Импорт категорий

Вы можете сделать отдельно импорт чисто категорий товаров. Файл импорта должен содержать как минимум поля — pagetitle, parent и поле которое указано в настройках импорта в опции **“Ключ товара, по которому определяется его уникальность”**
...
Если при импорте id главного и подчиненного товара могут не совпадать (файл импорта был получен в результате экспорта
...
Поддерживается импорт как в родную галерею minishop2, так и в MS2Gallery версии 2.0 или выше. При использовании MS2Gallery в качестве галереи для товаров следует учесть тот момент, что MS2Gallery синхронизирует изображения с родной галереей minishop2 то есть происходит дублирование информации об изображении в 2 таблице БД вследствие чего импорт происходит медленнее чем если бы вы использовали родную галерею minishop2. Если вы всё же решили использовать MS2Gallery то в настройках импорта следует указать это в опции «Тип галереи» выбрав в выпадающем списке «MS2Gallery».
...
- **file** — путь к изображению для галерее может быть как локальный (относительно указанного в настройках импорта
  опция **«Базовая директория для изображений»**) так и
  http/https ссылкой на сторонний ресурс (при использовании http/https ссылки импорт происходит медленнее)
...
Каждый из способов имеет свои плюсы и минусы. Первый стоит использовать когда вы уверены, что товар для которого вы делаете импорт модификаций уже существует. Второй стоит использовать, когда кода товара может ещё не быть. Минус 2-го варианта в том, что будет для каждой модификации одного и того же товара повторно будет происходить и обновление товара.
...
2. создать (если ещё нет) название для настройки полей, для этого необходимо нажать кнопку с шестерёнкой.

    ![создать (если ещё нет) название для настройки полей](https://file.modx.pro/files/8/c/a/8ca73abca4a2aa184237e0732e899780.jpg)
...
- **preset** — ID настройки полей экспорта. Если данный параметр не указан, то в файле экспорта будут следующие поля: pagetitle,price и href
...
Для того, чтобы сделать экспорт по cron необходимо дернуть следующую ссылку: `/assets/components/msimportexport/export/export.php`.
