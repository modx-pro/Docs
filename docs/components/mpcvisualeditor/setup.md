...
После этого появляется панель редактора, а редактируемые блоки на странице подсвечиваются. Клик по блоку открывает нужный редактор — подробнее в разделе [Редактирование полей](fields).

Панель по умолчанию закреплена **сверху**; её положение (вверху/внизу) задаётся настройкой `mpcve_toolbar_position`. Панель можно **свернуть** кнопкой на ней — она сворачивается в компактный хэндл в углу; состояние запоминается и сохраняется между перезагрузками.
...
| `mpcve_toolbar_position` | `top` | положение панели редактора: `top` (вверху) или `bottom` (внизу) |
...
| `mpcve_rte_entities` | *(пусто)* | состав палитры спецсимволов (кнопка «Ω») в редакторе форматированного текста: имена сущностей (`nbsp`, `mdash`) или числовые ссылки (`#8212`, `#x2014`) через запятую; пусто — набор по умолчанию |
