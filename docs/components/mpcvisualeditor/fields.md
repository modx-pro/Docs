...
В редакторе форматированного текста есть кнопка **«Ω»** — палитра популярных HTML-сущностей: неразрывный пробел и мягкий перенос, длинное и короткое тире, кавычки-ёлочки и лапки, многоточие, знак умножения, `±`, `°`, `©`, `®`, `™`, стрелка. Символ вставляется в место курсора, а если текст выделен — заменяет выделение. Состав палитры задаётся настройкой `mpcve_rte_entities` (см. [Установка и настройка](setup#системные-настройки)).
