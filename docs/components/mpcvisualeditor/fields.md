...
- **кнопка-триггер у блока** на странице открывает панель скрытых полей этого блока — это работает и для секции, и для конкретной строки списка (у строки со скрытыми полями появляется свой триггер);
- **из панели [«Секции»](sections)** (сайдбар): у каждой секции кнопка **✎ поля** открывает панель всех её полей, включая скрытые и вырезанные;
- **в окне [«Строки списка»](rows)**: у каждой строки кнопка **✎ поля** открывает все поля именно этой строки — видимые и скрытые.
