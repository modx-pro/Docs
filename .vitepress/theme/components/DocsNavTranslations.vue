...
import { useNavOverflow } from 'vitepress/dist/client/theme-default/composables/nav-overflow'
...
<template>
  ...
  <div v-if="screen && show" class="VPNavTranslations VPNavScreenTranslations" :class="{ open: isOpen }">
    <button type="button" class="title" :aria-expanded="isOpen" :aria-controls="listId" @click="toggle">
    ...
    <ul v-show="isOpen" :id="listId" class="list">
      <li v-for="locale in localeLinks" :key="locale.link" class="item">
        <VPLink class="link" :href="locale.link" :external="false">{{ locale.text }}</VPLink>
      </li>
    </ul>
  </div>
  ...
  <div v-else-if="menu && show" class="VPNavTranslations group translations">
    ...
    <ul>
      <template v-for="locale in localeLinks" :key="locale.link">
        <VPMenuLink :item="locale" :external="false" />
      </template>
    </ul>
  </div>

  <!-- inline flyout in the navbar -->
  <VPFlyout v-else-if="!menu && show" class="VPNavTranslations VPNavBarTranslations" :class="{ collapsed: isCollapsed }"
    icon="vpi-languages" :label="theme.langMenuLabel || 'Change language'"
